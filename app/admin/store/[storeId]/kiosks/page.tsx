    ...
    <div className="p-6 lg:p-10 max-w-7xl mx-auto">
      ...
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-slate-500 font-medium">
            Manage self-service kiosks for this store.
            <span className={`ml-3 inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-bold ${atLimit ? 'bg-rose-100 text-rose-700' : 'bg-slate-100 text-slate-600'}`}>
              {kiosks.length} / {maxKiosks > 0 ? maxKiosks : '∞'} kiosks
            </span>
          </p>
        </div>
        <button onClick={openAdd} disabled={atLimit}
          className="inline-flex items-center px-5 py-2.5 bg-red-600 text-white rounded-xl font-bold hover:bg-red-700 transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed">
          <Plus className="w-5 h-5 mr-2" /> Add Kiosk
        </button>
      </div>

      {/* ── Kiosk Sync / Publish Banner ─────────────────────────────────── */}
      <div className="mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-gradient-to-r from-slate-900 to-slate-800 border border-slate-700 rounded-2xl p-5 shadow-lg">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="font-black text-white text-base">Publish to All Kiosks</div>
            <div className="text-sm text-slate-400 mt-0.5">
              {lastPublished
                ? <>Last published: <span className="text-slate-300 font-bold">{lastPublished.toLocaleString()}</span></>
                : 'Triggers all active kiosks to reload menu & branding on next cycle.'}
            </div>
          </div>
        </div>
        <button
          ...
          className="inline-flex items-center gap-2 px-6 py-3 bg-red-600 hover:bg-red-500 text-white rounded-xl font-black transition-all shadow-md shadow-red-600/30 disabled:opacity-60 disabled:cursor-not-allowed flex-shrink-0"
        >
          {isPublishing
      ...
      {kiosks.length === 0 ? (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          ...
          <h3 className="text-xl font-bold text-slate-900 mb-2">No kiosks yet</h3>
          <p className="text-slate-500 mb-6">Add your first self-service kiosk terminal.</p>
          <button onClick={openAdd} disabled={atLimit}
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-red-600 text-white rounded-xl font-bold hover:bg-red-700 transition-colors disabled:opacity-50">
