...
const STATUS_TABS = ['New', 'Preparing', 'Ready', 'Out for Delivery'];
...
export default function StoreOrdersPage({ params }: { params: Promise<{ storeId: string }> }) {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      setSelectedOrder(prev => {
        if (prev) {
          ...
          // Auto-select first New order if none selected
          const firstNew = fetchedOrders.find(o => o.status === 'New');
  ...
  const assignDriverWithTime = async (orderId: string, driver: Driver, readyTime: string) => {
    try {
      ...
      // Blast Native Push Notification Backup to sleeping devices
      fetch('/api/notify-driver', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          driverId: driver.id,
          title: `New Assignment! (Ready ${readyTime})`,
          body: `Order #${selectedOrder?.orderNumber || orderId.substring(0,6).toUpperCase()} assigned to you.`,
          data: { orderId: orderId, type: 'assignment_pending' }
        })
      }).catch(e => console.log('Push notification warning:', e));
  ...
  const getActionConfig = (order: OrderType) => {
    switch (order.status) {
      case 'New':
        return { label: 'Accept & Print', color: 'bg-emerald-500 hover:bg-emerald-400 text-white', icon: Printer, next: 'Preparing', disabled: false };
      case 'Preparing':
  ...
  const renderOrderDetails = (order: OrderType) => {
    ...
    return (
      <div className="flex flex-col h-full bg-slate-50 font-sans">
          ...
          <div className="flex justify-between items-start mb-6">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <h2 className="text-4xl font-black tracking-tighter text-slate-900">#{order.orderNumber}</h2>
            ...
            <div className="text-right">
              <button 
                ...
                className="w-12 h-12 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-600 hover:text-slate-900 flex items-center justify-center transition-all shadow-sm no-print"
        ...
        <div className="flex-1 overflow-y-auto p-6 lg:p-10 relative">
          <div className="bg-white mx-auto max-w-2xl rounded-sm border border-slate-200 shadow-sm relative before:absolute before:-top-2 before:left-0 before:right-0 before:h-2 before:bg-[url('data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSIyMCIgaGVpZ2h0PSI4Ij48cGF0aCBkPSJNMCAwdjhsNS01IDUgNSA1LTUgNSA1VjB6IiBmaWxsPSIjZmZmZmZmIiBmaWxsLXJ1bGU9ImV2ZW5vZGQiLz48L3N2Zz4=')] before:bg-repeat-x after:absolute after:-bottom-2 after:left-0 after:right-0 after:h-2 after:bg-[url('data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSIyMCIgaGVpZ2h0PSI4Ij48cGF0aCBkPSJNMCA4VjBsNSA1IDUtNSA1IDUgNS01djh6IiBmaWxsPSIjZmZmZmZmIiBmaWxsLXJ1bGU9ImV2ZW5vZGQiLz48L3N2Zz4=')] after:bg-repeat-x z-0">
            <div className="px-6 py-8 relative shadow-[inset_0_0_40px_rgba(0,0,0,0.02)]">
              ...
              <div className="mt-8 pt-6 border-t-[3px] border-double border-slate-200 flex justify-between items-end">
                <span className="text-sm font-black text-slate-400 uppercase tracking-widest">Total Amount</span>
                <span className="text-4xl font-black text-slate-900 tracking-tighter">€{order.total.toFixed(2)}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 lg:p-8 border-t border-slate-200 bg-white flex-shrink-0 z-10 shadow-[0_-10px_40px_rgba(0,0,0,0.03)]">
          {action ? (
            <button
              ...
              className={`w-full py-6 rounded-3xl font-black text-2xl tracking-tight shadow-xl transition-all flex items-center justify-center gap-4 ${action.color} ${!action.disabled ? 'hover:scale-[1.02] hover:shadow-2xl active:scale-[0.98]' : ''} ${action.color.includes('amber') ? 'shadow-amber-500/30' : action.color.includes('emerald') ? 'shadow-emerald-500/30' : action.color.includes('blue') ? 'shadow-blue-500/30' : 'shadow-slate-900/20'}`}
            >
  ...
  return (
    <div className="flex flex-col lg:flex-row h-[calc(100vh-4rem)] lg:h-screen w-full bg-slate-100 overflow-hidden font-sans">
      
      {/* Left Panel: Order Queue */}
      <div className="w-full lg:w-[420px] bg-white border-r border-slate-200 flex flex-col h-full flex-shrink-0 z-20 shadow-2xl lg:shadow-none">
        {/* Header */}
                  ...
                  <span className="relative z-10 flex items-center justify-center gap-2 whitespace-nowrap text-sm">
                    ...
                    {count > 0 && (
                      <span className={`px-1.5 py-0.5 rounded-md text-[10px] font-black tracking-wider ${
                        isActive && tab === 'New' ? 'bg-amber-500 text-white animate-pulse' : 
      ...
      <div className="hidden lg:flex flex-1 h-screen overflow-hidden bg-slate-50 relative border-l border-slate-200">
            ...
            <tbody>
              {selectedOrder.items.map((item, idx) => (
                <tr key={idx} className="align-top">
                  ...
                  <td className="pt-1 pr-1 break-words">{item.name}</td>
